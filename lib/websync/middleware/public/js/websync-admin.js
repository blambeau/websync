...
websync = {
  ...
  ace: {
    ...
    cancel: function() {
      websync.ace.load(websync.ace.model.file, websync.ace.model.line);
    },
    save: function() {
      websync.ace.update({state: "saving"});
      $.ajax({
        type: "POST",
        url: "ace/save",
        data: { file: websync.ace.model.file,
               value: websync.ace.editor.getSession().getValue() },
        success: function(data) {
          websync.ace.update({
            value: data,
            state: "clean"
